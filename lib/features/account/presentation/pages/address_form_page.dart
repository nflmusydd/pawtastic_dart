...
class _AddressFormPageState extends State<AddressFormPage> {
  ...
  Future<void> _saveAddress() async {
    ...
    try {
      ...
      if (success && mounted) {
        final successMsg = widget.isShopPickup ? "${context.t.account.create_shop.successfully_saved_shop_data.ucfirst()}" : "${context.t.address.form.successfully_saved_address_data.ucfirst()}";
        SnackBarUtils.show(context, successMsg, type: SnackBarType.success);
        Navigator.pop(context);
      } else {
        final errorMsg = widget.isShopPickup ? "${context.t.account.create_shop.failed_to_save_shop_data.ucfirst()}" : "${context.t.address.form.failed_to_save_address_data.ucfirst()}";
  ...
  void _showSaveConfirmation() {
    ...
    final title = widget.isShopPickup ? "${context.t.address.form.change_shop_address.toTitleCase()}?" : "${context.t.address.form.save_address.toTitleCase()}?";
    final message = widget.isShopPickup 
        ? context.t.account.create_shop.make_sure_the_shop_data_and_pickup_address_are_correct.ucfirst()
        : context.t.common.make_sure_all_the_data_you_have_filled_in_is_correct.ucfirst();
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    CustomTextFieldRow(
                      ...
                      hintText: widget.isShopPickup ? t.shop_contact_name.toTitleCase() : t.recipient_name.toTitleCase(),
                      ...
                      DangerButton(
                        ...
                        onPressed: () {
                          if (widget.address!.isDefaultShipping) {
                            SnackBarUtils.show(context, t.cannot_delete_default_address.ucfirst(), type: SnackBarType.error);
                            return;
                          }
                          DialogUtils.showConfirmationDialog(
                            context: context,
                            ...
                            onConfirm: () async {
                              ...
                              if (mounted) {
                                ...
                                if (success) {
                                  SnackBarUtils.show(context, t.successfully_deleted_address_data.ucfirst(), type: SnackBarType.success);
                                  Navigator.pop(context);
                                } else {
                                  SnackBarUtils.show(context,t.failed_to_delete_address_data.ucfirst(), type: SnackBarType.error);
                                }
                              }
                            },
                          );
